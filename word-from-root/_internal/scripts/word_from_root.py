...
def _transform_export_value(column_def: dict, value, row: Optional[dict] = None):
    ...
    elif transform == "sem_trend_json":
        if isinstance(value, list):
            transformed = json.dumps(value, ensure_ascii=False, separators=(",", ":"))
        elif value in (None, ""):
            transformed = "[]"
            ...
            if not text:
                ...
                try:
                    ...
                    if isinstance(parsed, list):
                        transformed = json.dumps(parsed, ensure_ascii=False, separators=(",", ":"))
                    else:
                        transformed = text
        else:
            transformed = json.dumps(value, ensure_ascii=False)
    else:
        transformed = value
